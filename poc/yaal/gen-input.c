...
static uint8_t
trigger_byte(enum yaal_state to, uint64_t *prng)
{
	switch (to) {
	case YAAL_STATE_NL: return (uint8_t)'\n';
	case YAAL_STATE_ID: return (uint8_t)' ';
	case YAAL_STATE_VL: {
		/* Any byte that is neither '\n' nor ' '. */
		uint8_t b = (uint8_t)('a' + (sm64(prng) % 26));
		return b;
	}
	case YAAL_STATE_ST: break;
	}
	/* should not happen */
	return (uint8_t)'?';
}
 ...
 */
static int
emit_filler(uint8_t *out, size_t n, enum yaal_state state, uint64_t *prng)
{
	if (n == 0)
		return 0;
	switch (state) {
...
main(int argc, char **argv)
{
	...
	while ((opt = getopt(argc, argv, "i:o:s:t:h")) != -1) {
		switch (opt) {
		case 'i': trans_path = optarg; break;
		case 'o': out_path = optarg; break;
		case 's': seed = strtoull(optarg, NULL, 10); break;
		case 't': trail = strtoull(optarg, NULL, 10); break;
		case 'h':
	...
	struct yaal_transition *recs =
		malloc(IN_BUF_RECS * sizeof(*recs));
...
#define EMIT_BYTES(src, len) do { \
		...
		while (_remaining > 0) { \
			...
			_p += _take; \
			_remaining -= _take; \
			if (out_off == OUT_BUF_BYTES) FLUSH_OUT(); \
		} \
	} while (0)
	...
	for (;;) {
		...
		for (size_t i = 0; i < n; i++) {
			...
			if ((size_t)rec.offset < cursor) {
				fprintf(stderr,
					"trans not monotonic at rec %"
					PRIu64 ": offset %zu < cursor %"
					PRIu64 "\n",
					total_records - n + i,
					rec.offset, cursor);
				rc = 1;
			...
			if (rec.from != state) {
				fprintf(stderr,
					"trans state desync at rec %"
					PRIu64 ": from=%d expected=%d\n",
					total_records - n + i,
					(int)rec.from, (int)state);
				rc = 1;
				goto done;
			}
			...
			if (filler > 0) {
				...
				while (filler > 0) {
					...
					if (emit_filler(out + out_off, take,
							state, &prng) < 0) {
						fprintf(stderr,
							"emit_filler failed\n");
						rc = 1;
	...
	if (trail > 0) {
		if (state == YAAL_STATE_ST) {
			fprintf(stderr,
				"warning: trail>0 but final state is ST; skipping\n");
		} else {
			while (trail > 0) {
				size_t space = OUT_BUF_BYTES - out_off;
	...
	fprintf(stderr,
		"gen-input: read %" PRIu64 " transitions, wrote %"
		PRIu64 " input bytes\n  seed=%" PRIu64
		" final_state=%d\n",
