...
int main(int argc, char **argv)
{
    ...
    while ((opt = getopt(argc, argv, "n:o:s:f:h")) != -1) {
        ...
        case 's':
            ...
            break;
        case 'f':
            max_filler = (uint32_t)strtoul(optarg, NULL, 10);
            break;
        case 'h':
        default:
            usage(argv[0]);
    ...
    struct yaal_transition *buf = malloc(OUT_BUF_RECS * sizeof(*buf));
    if (!buf) {
    ...
    fprintf(stderr,
            ...
            emitted, emitted * (uint64_t)sizeof(struct yaal_transition), seed, max_filler,
            (int)state, offset);
    return 0;
}
