 ...
 */
static inline void
yaal_parser_step(struct yaal_parser *p, enum yaal_class c, size_t offset)
{
	enum yaal_state next =
		(enum yaal_state)yaal_transition_table[p->state][c];
...
static inline enum yaal_class
yaal_classify_byte(uint8_t b)
{
	if (b == '\n')
...
void yaal_parser_feed_scalar(struct yaal_parser *p,
			     const uint8_t *buf, size_t len);
void yaal_parser_feed_avx2(struct yaal_parser *p,
			   const uint8_t *buf, size_t len);
