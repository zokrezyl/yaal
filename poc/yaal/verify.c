...
static int
refill(struct verify_ctx *v)
{
	if (v->eof)
		return 0;
	ssize_t r = read(v->trans_fd, v->buf,
			 TRANS_BUF_RECS * sizeof(*v->buf));
	if (r < 0) {
		perror("read trans");
		v->mismatch = 1;
		return 0;
...
static void
verify_cb(void *vctx, size_t offset, enum yaal_state from, enum yaal_state to)
{
	...
	if (v->buf_pos == v->buf_count) {
		fprintf(stderr,
			...
			offset, state_name(from), state_name(to),
			v->total_compared);
		v->mismatch = 1;
		return;
	}
	...
	if (expected.offset != offset || expected.from != from || expected.to != to) {
		fprintf(stderr,
			...
			"  parser:   off=%zu %s->%s\n",
			v->total_compared,
			expected.offset, state_name(expected.from),
			state_name(expected.to),
			offset, state_name(from), state_name(to));
		v->mismatch = 1;
		return;
	}
...
main(int argc, char **argv)
{
	...
	while ((opt = getopt(argc, argv, "i:t:c:b:h")) != -1) {
		...
		case 't': trans_path = optarg; break;
		case 'c':
			...
			break;
		case 'b': chunk_kib = strtoul(optarg, NULL, 10); break;
		case 'h':
	...
	if (rc == 0) {
		...
		if (vctx.buf_pos < vctx.buf_count) {
			fprintf(stderr,
				"verify: parser produced %" PRIu64
				" events, but ground truth has more\n",
				vctx.total_compared);
			rc = 1;
		} else {
			ssize_t r = read(trans_fd, vctx.buf,
					 sizeof(*vctx.buf));
			if (r > 0) {
				fprintf(stderr,
					"verify: ground truth has more events than parser\n");
				rc = 1;
			} else if (r < 0) {
