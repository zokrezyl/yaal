...
classify_chunk_avx2(__m256i chunk)
{
...
static void
feed_avx2_emit(struct yaal_parser *p, const uint8_t *buf, size_t len)
{
...
#define APPLY_F(F, state) \
	((enum yaal_state)apply_function_table[((F) << 2) | (state)])
...
static const uint8_t compose_table[16] = {
	/* prev=0 (f_nl):     cur=0..3 -> cur ∘ f_nl    */ 0, 3, 2, 3,
	/* prev=1 (f_sp):     cur=0..3 -> cur ∘ f_sp    */ 0, 1, 2, 3,
	/* prev=2 (f_ns):     cur=0..3 -> cur ∘ f_ns    */ 0, 2, 2, 3,
	/* prev=3 (f_to_id):  cur=0..3 -> cur ∘ f_to_id */ 0, 3, 2, 3,
};

static inline uint8_t
fcompose(uint8_t a, uint8_t b)	/* returns b ∘ a — a applied first */
{
...
static inline uint8_t
chunk_function_id_bitmask(__m256i chunk)
{
 ...
 */
static void
feed_avx2_silent(struct yaal_parser *p, const uint8_t *buf, size_t len)
{
	...
	for (; i < head_end; i++)
		state = (enum yaal_state)yaal_transition_table
			[state][yaal_classify_byte(buf[i])];
	 ...
	 */
	size_t unroll2_end =
		i + ((body_end - i) & ~(size_t)(2 * YAAL_AVX2_CHUNK - 1));

	for (; i < unroll2_end; i += 2 * YAAL_AVX2_CHUNK) {
		if (i + 256 < unroll2_end)
			_mm_prefetch((const char *)(buf + i + 256),
				     _MM_HINT_T0);
...
void
yaal_parser_feed_avx2(struct yaal_parser *p, const uint8_t *buf, size_t len)
{
	if (p->cb)
