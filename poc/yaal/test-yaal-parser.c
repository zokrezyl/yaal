...
static void
collect_cb(void *vctx, size_t offset, enum yaal_state from, enum yaal_state to)
{
...
static const char *
state_name(enum yaal_state s)
{
	switch (s) {
	case YAAL_STATE_ST: return "ST";
	case YAAL_STATE_NL: return "NL";
	case YAAL_STATE_ID: return "ID";
	case YAAL_STATE_VL: return "VL";
	}
	return "??";
}

static int
run_one(const uint8_t *buf, size_t len, enum yaal_impl impl,
	struct collector *c)
{
	c->count = 0;
	c->overflow = 0;
...
static int
diff(const struct collector *a, const struct collector *b,
	...
	for (size_t i = 0; i < a->count; i++) {
		...
		if (x->offset != y->offset || x->from != y->from || x->to != y->to) {
			fprintf(stderr,
				...
				"  avx2:   off=%zu %s->%s\n",
				label, i,
				x->offset, state_name(x->from), state_name(x->to),
				y->offset, state_name(y->from), state_name(y->to));
			return 1;
		}
	}
	return 0;
...
static enum yaal_state
final_state_silent(const uint8_t *buf, size_t len, enum yaal_impl impl)
{
...
static int
diff_silent(const uint8_t *buf, size_t len, const char *label,
	    const struct collector *scalar_cb)
{
	enum yaal_state from_scalar_cb = scalar_cb->count == 0
		? YAAL_STATE_ST
		: scalar_cb->evs[scalar_cb->count - 1].to;
	...
	if (from_scalar_cb != silent_avx2) {
		fprintf(stderr,
			...
			label,
			state_name(from_scalar_cb),
			state_name(silent_avx2),
			len);
		return 1;
	}
	return 0;
...
static int
run_diff(struct buffers *bx, const uint8_t *buf, size_t len, const char *label)
{
...
static int
run_at_alignment(struct buffers *bx, const uint8_t *payload, size_t plen,
...
static int
test_crafted(struct buffers *bx)
{
	struct case_def {
		const char *label;
		const char *text;
	...
	for (size_t i = 0; i < sizeof(cases) / sizeof(cases[0]); i++) {
		...
		for (size_t off = 0; off < 32; off++) {
			if (run_at_alignment(bx, (const uint8_t *)cases[i].text,
					     plen, off, cases[i].label))
				rc = 1;
		}
	}
	return rc;
...
static int
test_fuzz(struct buffers *bx, size_t iters)
{
	...
	static const size_t lens[] = {
		0, 1, 2, 7, 15, 16, 17, 31, 32, 33,
		47, 48, 49, 63, 64, 65, 95, 96, 97,
		127, 128, 200, 257, 511, 512, 513, 1024, 4096,
	};
	...
	for (size_t it = 0; it < iters; it++) {
		...
		for (size_t i = 0; i < plen; i++)
			target[i] = random_byte(&seed);
		char sub[64];
		snprintf(sub, sizeof(sub), "fuzz-rand it=%zu off=%zu len=%zu",
			 it, off, plen);
		if (run_diff(bx, target, plen, sub))
			return 1;
	}
...
main(void)
{
	...
	int rc = 0;
	if (test_crafted(&bx)) rc = 1;
	if (test_fuzz(&bx, 256)) rc = 1;
